...
from shop.models import Order,Product,Variation
...
def userregister(request):
    ...
    if request.method == 'POST':
        ...
        if User.objects.filter(email=email).exists():
            ...
            context['password'] = 'Your password must contain at least 8 charcters.'
...
def orders(request):
    ...
    for order in orders:
        for o in order.order_list:
            if o['type'] == 'product':
                order_list.append({'product':Product.objects.get(id=o['product_id']),'price':o['price'],'quantity':o['quantity'],'size':'','transaction_id':order.transaction_id,'address':order.address,'pincode':order.zipcode,'city':order.city,'state':order.state,'contact_number':order.contact_number,'type':'product','status':order.status})
            else:
                order_list.append({'product':Variation.objects.get(id=o['product_id']),'price':o['price'],'quantity':o['quantity'],'size':o['size'],'transaction_id':order.transaction_id,'address':order.address,'pincode':order.zipcode,'city':order.city,'state':order.state,'contact_number':order.contact_number,'type':'variation','status':order.status})
    return render(request,'account/orders.html',{'orders':orders,'order_list':order_list})
