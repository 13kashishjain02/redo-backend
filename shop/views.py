...
from .helpers import exist_wishlist, Total_MRP
...
def cart(request):
    ...
    total_amount = total_mrp - int(code)
    return render(request,'shop/cart.html',{'products':products,'products_v':products_v,'mrp':total_mrp,'total_amount':total_amount})
...
def address(request):
    if Cart.objects.filter(user=request.user).exists():
        if request.method == "POST":
            ...
            change_address = Cart.objects.filter(user=request.user)
            for change_adrs in change_address:
                change_adrs.address = Address.objects.get(id=address)
                change_adrs.save()
        ...
        total_mrp = Total_MRP(request)
        total_amount = total_mrp - int(code)
        return render(request,'shop/address.html',context={'address':address,'products':products,'mrp':total_mrp,'total_amount':total_amount,'products_v':products_v})
...
def payment(request):
    if Cart.objects.filter(user=request.user).exists():
        ...
        total_mrp = Total_MRP(request)
        total_amount = total_mrp - int(code)
        return render(request,'shop/payment.html',context={'mrp':total_mrp,'total_amount':total_amount})
    return redirect('/checkout/cart/')
...
def products(request,category=None, subcategory1=None,subcategory2=None,subcategory3=None):     
    if category is not None and subcategory1 is None and subcategory2 is None and subcategory3 is None:
        ...
        for i in products_v:
            if i.product.category == category and i.product.has_variation:
                p_v.append(i)
        return render(request,'shop/product.html',context={'products':products,'category':category,'products_v':p_v})
        ...
        products_v = Variation.objects.all()
        for i in products_v:
            if i.product.subcategory1 == subcategory1 and i.product.has_variation:
                p_v.append(i)
        return render(request,'shop/product.html',context={'products':products,'category':category,'subcategory1':subcategory1,'products_v':p_v})
        ...
        products_v = Variation.objects.all()
        for i in products_v:
            if i.product.subcategory2 == subcategory2 and i.product.has_variation:
                p_v.append(i)
        return render(request,'shop/product.html',context={'products':products,'category':category,'subcategory1':subcategory1,'subcategory2':subcategory2,'products_v':p_v})
...
def change_quantity(request,id):
    if request.method == "POST":
        ...
        total_mrp = Total_MRP(request)
        total_amount = total_mrp - int(code)
    return JsonResponse({'price':float(int(cart.product.special_price) * int(cart.quantity)),'total_mrp':total_mrp,'total_amount':total_amount},safe=False)
...
def move_to_cart(request,id):
    wishlist = My_Wishlist.objects.get(id=id)
    if Cart.objects.filter(user=request.user,product=wishlist.product.id):
            messages.error(request,'This product already present in cart !!')
            return redirect('/wishlist/')
    else:
        Cart.objects.create(
        user=request.user,
        product = Product.objects.get(id=wishlist.product.id)
    )
    wishlist.delete()
    return redirect('/checkout/cart/')
...
def productDetailsVariation(request,id):
    ...
    if request.method == "POST" and 'cart' in request.POST:
        size = 'size' in request.POST and request.POST['size']
        if Cart.objects.filter(product_variation=Variation.objects.get(id=id),size=size).exists():
            messages.error(request,'Already to cart !!')
        else:
            Cart.objects.create(
                user= request.user,
                size = size,
                product_variation = Variation.objects.get(id=id),
            )
            messages.success(request,'Added to cart !!')

    return render(request,'shop/product-details-variation.html',{'product':product,'product_details':product_details,'size':size})

def productDetailsVariationCart(request,id):
    product = Variation.objects.get(id=id)
    product_details = Variation.objects.filter(product=product.product)
    if request.method == "POST" and 'cart' in request.POST:
        size = 'size' in request.POST and request.POST['size']
        if Cart.objects.filter(product_variation=Variation.objects.get(id=id),size=size).exists():
            ...
            messages.success(request,'Added to cart !!')
        return redirect(f'/variation/{id}/buy/')

def change_quantity_variation(request,id):
    if request.method == "POST":
        quantity = request.POST.get('quantity')
        cart = Cart.objects.get(id=id)
        cart.quantity = quantity
        ...
        total_mrp = Total_MRP(request)
        total_amount = total_mrp - int(code)
    return JsonResponse({'price':float(int(cart.product_variation.special_price) * int(cart.quantity)),'total_mrp':total_mrp,'total_amount':total_amount},safe=False)

def move_to_wishlist_variation(request,id):
    variation = Variation.objects.get(id=id)
    print(variation)
    return redirect(f'/variation/{id}/buy/?color={variation.color}')

def apply_coupon(request):
    data = {}
    coupon_code = request.POST.get('coupon')
    if Coupons.objects.filter(code=coupon_code,used=False).exists():
        request.session['coupon_code'] = str(Coupons.objects.get(code=coupon_code))
        request.session['discount'] = Coupons.objects.get(code=coupon_code).discout
        data['success'] = request.session['discount']
        print(data)
    else:
        data['invalid'] = 'Sorry !! Invalid coupon code'
    code = 0
    if 'discount' in request.session:
        code = request.session['discount']
    total_mrp = Total_MRP(request)
