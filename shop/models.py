...
from account.models import VendorAccount,Account,Address
...
class Variation(models.Model):
    ...
    def Weigth(self):
        weigth = Product.objects.get(id=self.product.id)
        length = Product.objects.get(id=self.product.id)
        width = Product.objects.get(id=self.product.id)
        height = Product.objects.get(id=self.product.id)
        weigth_kg = weigth.weight
        volumetric_weight = (length.length * width.width * height.height)/4000
        if weigth_kg > volumetric_weight:
            weigth = weigth_kg
        else:
            weigth = volumetric_weight
    ...
    def sales_value(self):
        gst = Product.objects.get(id=self.product.id)
        return (self.special_price*100)/(100+int(gst.gst))
...
class Cart(models.Model):
    ...
    product_variation = models.ForeignKey(Variation,on_delete=models.CASCADE,null=True,blank=True)
